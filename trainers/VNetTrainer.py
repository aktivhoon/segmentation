...
class VNetTrainer(BaseTrainer):
	def __init__(self, arg, G, torch_device, recon_loss):
		super(VNetTrainer, self).__init__(arg, torch_device)
		...
		with torch.no_grad():
			...
			for i, (input_, target_, f_name) in enumerate(test_loader):
				...
				for batch_idx in range(0, input_.shape[0]):
					...
					utils.voxel_save(save_path, input_b, target_b, output_b)
